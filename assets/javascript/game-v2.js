...
var game = {
    ...
    updateWord: function(idx, l) {
        document.getElementById("u-" + idx).textContent = l;
        this.letterCount++;
    },
    isUsedLetter: function(key) {
        return this.usedLettersList.includes(key);
    },
    updateUsedLetters: function(l) {
    ...
    updateGuessesRemaining: function() {
        guessesDisplay.textContent = this.guessCount;
    },
    updateMan: function() {
    ...
    do_stuff: function(letter) {
        var index = [],
            letter = letter.toUpperCase();
        this.word = this.word.toUpperCase();
        // Check for multiple occurences
        for (var i = 0; i < this.word.length; i++) {
            var character = this.word.charAt(i);
            if (character == letter) {
                index.push(i);
            }
        }
        ...
        if (indexLength > 0) {
            ...
            if (indexLength > 1) {
                for (var x = 0; x < indexLength; x++) {
                    this.updateWord(index[x], letter);
                }
            } else {
                this.updateWord(index[x], letter);
            }
...
function game_setup(word) {
    ...
    // Set/Reset Game
    usedLettersDisplay.innerHTML = game.usedLettersList;
    // Reset canvas
